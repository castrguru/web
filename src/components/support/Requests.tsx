...
export default function Requests() {
    return (
        <>
            <h2 className="py-3 text-3xl font-bold text-slate-600 tracking-wider">
                Community Requests
            </h2>

            <ul role="list" className="divide-y divide-gray-100">
                {discussions.map((discussion) => (
                    <li key={discussion.id} className="flex flex-wrap items-center justify-between gap-x-6 gap-y-4 py-5 sm:flex-nowrap">
                        <div>
                        ...
                        <dl className="flex w-full flex-none justify-between gap-x-8 sm:w-auto">
                            <div className="flex -space-x-0.5">
                                ...
                                {discussion.commenters.map((commenter) => (
                                    <dd key={commenter.id}>
                                        <img alt={commenter.name} src={commenter.imageUrl} className="size-6 rounded-full bg-gray-50 ring-2 ring-white" />
                                    </dd>
                                ))}
                            </div>
                            <div className="flex w-16 gap-x-2.5">
                                <dt>
                                    ...
                                    {discussion.status === "resolved" ? <CheckCircleIcon aria-hidden="true" className="size-6 text-gray-400" /> : <ChatBubbleLeftIcon aria-hidden="true" className="size-6 text-gray-400" />}
                                </dt>
